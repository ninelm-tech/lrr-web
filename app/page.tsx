    ...
    <main>
      <section className="landing-hero">
        ...
        <div className="cta-buttons">
          <a href="/register/customer" style={{ background: "#003DB4", color: "#fff", fontWeight: 700 }}>
            Get Covered →
          </a>
      ...
      <section style={{ textAlign: "center", padding: "3rem 1rem", background: "#F6FAFF" }}>
        <h2 style={{ color: "#003DB4", fontSize: "1.6rem", fontWeight: 700, marginBottom: "0.5rem" }}>
          How it works
        </h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1.5rem", maxWidth: 800, margin: "1.5rem auto 0" }}>
          {[
            ...
            <div key={n} style={{ background: "#fff", borderRadius: 12, padding: "1.5rem", border: "1px solid #dde8f8", boxShadow: "0 2px 8px rgba(0,61,180,0.07)" }}>
              <div style={{ width: 36, height: 36, background: "#dde8f8", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#003DB4", margin: "0 auto 0.75rem" }}>{n}</div>
              <h4 style={{ margin: "0 0 0.4rem 0", color: "#333" }}>{title}</h4>
              <p style={{ margin: 0, color: "#666", fontSize: "0.9rem" }}>{desc}</p>
            </div>
          ))}
        </div>
        <a href="/register/customer" style={{ display: "inline-block", marginTop: "2rem", padding: "0.85rem 2rem", background: "#003DB4", color: "#fff", borderRadius: 8, fontWeight: 700, fontSize: "1rem", textDecoration: "none", boxShadow: "0 2px 12px rgba(0,61,180,0.2)" }}>
          Create Free Account →
        </a>
      </section>
    </main>
